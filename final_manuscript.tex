...
\begin{itemize}
    ...
    \item \textbf{Lensing / PPN:} Dynamical vector $A^\mu$ shear cancels scalar trace discrepancies, maintaining $\Phi = \Psi$ ($\gamma_{\text{PPN}} = 1$) without disformal distortion $\mathbf{[P]}$.
...
The Lean~4 suite (\texttt{05\_lean\_formalization/}) comprises \textbf{17 modules}, 0 \texttt{sorry}, axioms $\{\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}\}$, Lean \texttt{v4.33.0-rc1}, Mathlib pinned at \texttt{5eec30bc}. Gate: \texttt{verify\_all\_proofs.sh}.

\textbf{Important provenance note:} Local PASS on \texttt{109d38b}, Mathlib already present; O6 unwalked. Artifact: \texttt{VERIFICATION\_RUN\_003/01\_lean/} (transcript, per-target exit codes, toolchain and Mathlib revisions, SHA-256 checksums). Cold-clone reproducibility (\texttt{lake exe cache get} on a blank machine) is documented as open problem O6 in \texttt{OPEN\_PROBLEMS\_AND\_TESTS.md} and is not a current claim.
...
\midrule
\texttt{DualChannelDerivation.lean} & $\mathcal{F}_{\text{dual}}'(x) = x/(1+x)$; single-channel no-go $\mathbf{[P]}$. Physical mechanism of channel balance $\mathbf{[O]}$. \\
\texttt{PPNLimits.lean} & Newtonian and deep-MOND limits $\mathbf{[P]}$. \\
\texttt{RelativisticStability.lean} & Kinetic convexity $\mathcal{F}'' > 0$ $\mathbf{[P]}$. \\
\texttt{CovariantCompletion.lean} & RAQUAL superluminality; FLRW decoupling $\mathbf{[P]}$. Ghost freedom (4D) conditional $\mathbf{[P\text{-cond}]}$. \\
\texttt{TensorSpeed.lean} & $c_{13} = 0$ identity; disformal falsification; $\alpha_1 = -2K$ $\mathbf{[P]}$. \\
\texttt{SkordisZlosnikEmbedding.lean} & Exact $\mathcal{J}(\mathcal{Y})$ embedding; $c_T = c$; $\gamma_{\text{PPN}} = 1$ $\mathbf{[P]}$. \\
\texttt{PPNParameters.lean} & PPN parameter suite $\mathbf{[P]}$. \\
\texttt{MuProjection.lean} & Mathematical properties of $\mu(x)$ $\mathbf{[P]}$. Physical derivation of closure $\mathbf{[O]}$. \\
\texttt{SOCasimirGenuine.lean} & Casimir eigenvalues $\mathbf{[P]}$. Physical gauge symmetry $\mathbf{[O]}$. \\
\texttt{DeSitterExtremal.lean} & Horizon lapse vanishing $\mathbf{[P]}$. $a_0 = cH_0/(2\pi)$ derivation $\mathbf{[O]}$. \\
\texttt{ITActionClosure.lean} & Polynomial $\tau$-law / simple-$\mu$ relation $\mathbf{[P]}$. Relativistic stability $\mathbf{[O]}$. \\
\texttt{YettParadigm.lean} & Spectral gap positivity (idealized) $\mathbf{[P]}$. Physical vacuum states $\mathbf{[O]}$. \\
\texttt{SovereignRegularity.lean} & Conditional BKM boundedness \cite{BealeKatoMajda1984} $\mathbf{[P]}$. Global NS regularity $\mathbf{[O]}$. \\
\texttt{HorizonThermodynamics.lean} & KMS temperature matching $\mathbf{[P]}$. $1/(2\pi)$ divisor origin $\mathbf{[O]}$. \\
\texttt{InformationTension.lean} & Information-geometric identity $\mathbf{[P]}$. \\
\texttt{GalacticDynamics.lean} & Rotation curve asymptotics from $\mu(x)$ $\mathbf{[P]}$. \\
\texttt{CosmologicalBoundary.lean} & FLRW decoupling; $\Omega_\Lambda = \ln 2$ is boundary condition $\mathbf{[P/O]}$. \\
\bottomrule
\end{tabularx}
\caption{All 17 Lean~4 modules. Physical-theory selection is never certified; Lean certifies only that mathematical statements follow from their encoded definitions.}
...
\begin{enumerate}
    ...
    \item \textbf{O6 --- Cold-Clone Lean Build:} \texttt{lake exe cache get} on a blank machine not yet demonstrated.
\end{enumerate}
...
All raw data, Python scripts, Lean~4 modules, and verification manifests are archived in the repository at \url{https://github.com/Mega-Therion/Res-Nova}, verified artifact references \texttt{VERIFICATION\_RUN\_003}, \texttt{VERIFICATION\_RUN\_005}, \texttt{VERIFICATION\_RUN\_006}.
