...
\title{\textbf{Dual-Channel Variational Closure, Covariant Completion, and a Reproducible SPARC Benchmark:\\A Res-Nova v1.6.0 Technical Assessment}}
...
\author{
    ...
    \small Repository: \href{https://github.com/Mega-Therion/Res-Nova}{\texttt{github.com/Mega-Therion/Res-Nova}} --- Release: \texttt{v1.6.0}
}
...
\begin{abstract}
We present the Res-Nova v1.6.0 technical assessment of a dual-channel variational action for modified gravity in the AQUAL/MOND family. \textbf{Core result (proved):} The single-channel arcsinh action is falsified on correspondence grounds $\mathbf{[P]}$; the dual-channel potential $\mathcal{F}_{\text{dual}}(x) = \tfrac{1}{2}x^2 - x + \ln(1+x)$ yields exactly $\mu(x) = x/(1+x)$ with correct Newtonian ($\mu \to 1$) and deep-MOND ($\mu \to x$) limits, proved algebraically and machine-checked in Lean~4. \textbf{Conditional uniqueness (proved):} Given four structural constraints --- constitutive-relation form, Pad\'e$[1/1]$ minimality, the MOND boundary conditions, and dual-channel splitting --- $\mathcal{F}_{\text{dual}}$ is the \emph{unique} admissible potential $\mathbf{[P]}$; $\mu$ is moreover the functional inverse of the Bayesian odds ratio, and the action satisfies the Fisher identity $\mathcal{F}_{\text{dual}}'(x)^2\,\mathcal{I}(\mu(x)) = x^3$ with $\mathcal{I}$ the Bernoulli Fisher information. The \emph{necessity} of the Pad\'e constraint is not derived and remains open $\mathbf{[O]}$, so this is conditional, not unconditional, uniqueness. \textbf{Relativistic completion (proved):} Pure RAQUAL is falsified by superluminal characteristic cones $\mathbf{[P]}$; the disformal completion is falsified by GW170817 $\mathbf{[P]}$; the dual-channel action embeds exactly into the Skordis--Z\l{}o\'snik (2021) relativistic MOND framework with $c_T = c_\gamma = c$ and $\gamma_{\text{PPN}} = 1$ preserved identically $\mathbf{[P]}$. \textbf{Empirical benchmark (computed):} A properly bootstrapped $a_0$ measurement across 171 SPARC galaxies (3,375 kinematic points) yields $a_0 = (1.116 \pm 0.128_{\text{stat}} \pm 0.097_{\text{syst}}) \times 10^{-10}\,\text{m\,s}^{-2}$ (14.4\% total), consistent with $cH_0/(2\pi)$ at $0.46\sigma$ and MOND's canonical value at $0.52\sigma$; these two anchors are not separated at $z=0$. A matched-parameter comparison shows the dual-channel MOND tier requires 342 fewer free parameters than a cosmologically-constrained NFW halo model at comparable or better fit quality. \textbf{Formal verification:} 17 Lean~4 modules, 0 \texttt{sorry}, axioms restricted to \texttt{\{propext, Classical.choice, Quot.sound\}}. \textbf{Redshift test (computed):} A pre-registered $a_0(z)$ comparison on 20 MUSE-DARK III galaxies ($0.41 \le z \le 1.44$) favours constant $a_0$ over the horizon-tied $a_0(z) = \xi c H(z)$ at $5.9\sigma$ ($\Delta\chi^2 = -35.1$), excluding the horizon evolution hypothesis at $> 3\sigma$ $\mathbf{[D]}$. \textbf{Covariant completion and screening (computed):} Specifying the Skordis--Z\l{}o\'snik free function as $\mathcal{F}(\mathcal{K}) = \mathcal{F}_{\text{dual}}(\sqrt{\mathcal{K}})$ leaves the Friedmann background unmodified and places the cosmological background on the Newtonian branch, giving a linear screening ratio $\mathcal{F}''/\mathcal{F}' \approx 0.004$: the $\sim76\times$ structure overproduction of a uniform MOND enhancement is reduced to a $0.4\%$ enhancement of linear growth $\mathbf{[P]}$. Combined with Vainshtein suppression $(r/r_{\text{MOND}})^{3/2}$, the solar-system external-field quadrupole is $Q_2 \approx 4.9\times10^{-29}\,\text{s}^{-2}$, a factor $70$ below the Cassini bound of Park et al.~(2026) for natural $\mathcal{O}(1)$ couplings and with no fine-tuning $\mathbf{[D]}$; the residual $Q_2 \sim 10^{-29}\,\text{s}^{-2}$ is a falsifiable prediction for next-generation ranging. \textbf{Stability:} the completion is ghost-free with bounded Hamiltonian and $c_T=c$ $\mathbf{[P]}$. \textbf{Open items:} The $a_0$--horizon $1/(2\pi)$ normalization (disfavoured by our own O4 test), the necessity of the Pad\'e constraint, non-linear structure formation, exact PPN $\gamma$ and $\beta$, $\Omega_\Lambda = \ln 2$, and cluster-scale dynamics remain quarantined as $\mathbf{[O]}$ pending future derivation, simulation, and data.
\end{abstract}
