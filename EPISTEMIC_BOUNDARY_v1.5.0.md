...
| **D3.1** | `lim_{x→∞} μ(x)=1` | `[P]` | `05_lean_formalization/PPNLimits.lean` (`mu_newtonian_limit`) | **Closed 2026-09-09; the 2026-08-16 suspension is lifted.** The stated closure path — write the limit — is executed: `mu_newtonian_limit : Filter.Tendsto (fun x => mu x) atTop (𝓝 1)`, proved from `fractional_deviation_eq`, so it depends on `μ(x) = x/(1+x)` (substituting another interpolation function breaks it). Suspended-history note: until 2026-09-09 the suite contained only the finite-threshold bounds `solar_system_precision_bound` (x ≥ 10⁴), `cassini_radar_delay_satisfied` (x ≥ 6×10⁷), `messenger_perihelion_satisfied` (x ≥ 3×10⁸); those bounds remain valid and the new limit does not replace them. |
...
| **D4.1** | Old in-sample headline: 176 parameters, median `χ²/N_g=2.92`, `a0=(9.433±0.050)×10^{-11}` | **SUPERSEDED** | `res_nova_manuscript.tex`; older run manifests | Method defect: radial points treated as independent; 5-fold CV leaked a global `a0`. Keep as history. Do not quote as current. |
...
| **D4.3** | Working measurement: `a0=1.116×10^{-10} ± 0.128×10^{-10}` (stat) `± 0.097×10^{-10}` (syst); total 14.4%; `N=171` galaxies, 3375 points — **SUPERSEDED 2026-09-17, provenance only; live row is μ_std, a₀=1.1607e-10 on 175 galaxies (`docs/A0_CLOSURE_EVOLUTION.md`)** | `[D]` | `02_galaxy_dynamics/A0_MEASUREMENT.json`; `a0_measure.py` | Bootstrap over galaxies. Systematics: distance 6.6–6.9%, inclination 5.5%, M/L <1%. |
