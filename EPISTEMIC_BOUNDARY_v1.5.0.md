...
Standard: `[P]` proved, `[D]` direct empirical, `[C]` cited, `[O]` open / quarantined.
...
| **D3.1** | `lim_{x→∞} μ(x)=1` | `[P]` | `PPNLimits.lean` | Newtonian recovery of this `μ` only. |
| **D3.2** | `γ_PPN=1` in the Skordis–Złośnik parent | `[P]`-cond | `CovariantCompletion.lean`; `TARGET_D7_COVARIANT_COMPLETION.md` | Conditional on that embedding, not a generic scalar completion. |
...
| **F7** | Lean suite: 17 modules, 0 `sorry`, axioms `{propext, Classical.choice, Quot.sound}` | `[P]` | `05_lean_formalization/verify_all_proofs.sh` | Last reported PASS used a local Mathlib build. Fresh `lake exe cache get` is not claimed. v1.4.0 item F7 said “8 modules”; that count is obsolete. |
