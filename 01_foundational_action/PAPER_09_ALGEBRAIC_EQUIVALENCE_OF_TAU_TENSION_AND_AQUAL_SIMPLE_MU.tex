...
\author{{\color{ink}\large R.W. Yett} \\ {\color{cyan}\small Arkansas} % Research}}
...
\end{equation}
While $2.14 \times 10^{-6} < 2.3 \times 10^{-5}$ technically clears the Cassini $\gamma_{\text{PPN}}$ bound, it clears it by only a factor of $\sim 10\times$. \textbf{Correction (2026-09-12):} this fractional-smallness argument, while correct for $\gamma_{\text{PPN}}$ specifically, does not bound the dual-channel branch's actual dominant failure mode. An independent re-derivation (see \texttt{Res-Nova/MU\_DUAL\_SOLAR\_SYSTEM\_RECONCILIATION\_2026-09-12.md}) found the $1/x$ tail of $\mu=x/(1+x)$ forces a constant, unscreened anomalous acceleration producing Mercury perihelion precession $\sim 1000\times$ the observed bound — the orbital lever arm $r^2/(GM)$ converts the same $\sim10^{-6}$ fractional effect the table above reports into an absolute-acceleration violation, not a $10\times$-margin pass. The table's qualitative conclusion (single-channel $\mu_{\text{simple}}$ strongly preferred) is unaffected and independently corroborated by this later, more rigorous derivation — but "$10\times$ margin, technically clears" understates how decisively the dual-channel branch fails.
...
\item Yett, R. W. (2026). \emph{Ars Magna of Geometrically Ordered Dynamics}. Canonical Research Corpus. % Research.
