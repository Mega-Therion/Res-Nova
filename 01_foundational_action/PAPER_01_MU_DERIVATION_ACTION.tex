...
{\color{ink}\large R.W.\ Yett}\, {\color{dim}\small(Architect)}\\[0.2cm]
...
{\color{dim} R.W. Yett\,$^{1}$}\\[0.2em]
{\color{dim}\small $^{1}$ Arkansas \quad ORCID 0009-0001-1303-7190}\end{center}
\vspace{0.5em}
...
\end{equation}
While empirical fits—such as the standard function $\mu_{\text{standard}}(x) = x/(1+x)$ or the simple function $\mu_{\text{simple}}(x) = x/\sqrt{1+x^2}$—reproduce observed disk kinematics across diverse morphological types, the fundamental origin of $\mu(x)$ has remained a central open problem in theoretical astrophysics. \textbf{Terminology note (2026-09-12), not a physics correction:} this paper's naming is inverted relative to the rest of the Res-Nova corpus as of this date. Elsewhere, $x/(1+x)$ is called ``$\mu_{\text{dual}}$'' (found, 2026-09-12, to be falsified for solar-system use — see \texttt{Res-Nova/MU\_DUAL\_SOLAR\_SYSTEM\_RECONCILIATION\_2026-09-12.md}) and $x/\sqrt{1+x^2}$ is called ``$\mu_{\text{std}}$'' (the function this paper actually derives and uses throughout). \textbf{This paper's own derivation and result are unaffected} — $\mu_{\text{simple}}$ here is the same, surviving function as $\mu_{\text{std}}$ elsewhere, derived independently via a different route (the Conformal Sovereign Action, not the AQUAL/AeST dual-channel embedding). Flagged only so a name-based search-and-replace across the corpus does not invert this paper's meaning. In standard approaches, $\mu(x)$ is selected post hoc to fit observational databases such as SPARC \cite{SPARC2016}, lacking a origin in a 4D covariant action principle.
...
\bibitem{ChyrenCodex2026} R. W. Yett, \emph{Geometrically Ordered Dynamics: Canonical Codex and Zenodo Repository}, DOI: 10.5281/zenodo.21316842 (2026).
\end{thebibliography}
