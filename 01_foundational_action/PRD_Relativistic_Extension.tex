...
\begin{equation}
    g_{\mu\nu} A^\mu A^\nu = -1
\end{equation}
The coherence of this geometric phase across spacetime is captured by the scalar alignment field $\chi(x^\mu)$. Rather than being introduced as an ad-hoc scalar field, $\chi$ descends group-theoretically from the exceptional Lie algebra $\mathfrak{e}_8$. Under the maximal subgroup $\mathrm{Spin}(16)/\mathbb{Z}_2 \subset E_8$, the adjoint representation decomposes into adjoint and spinor sectors:
\begin{equation}
    \mathbf{248} = \mathbf{120} \oplus \mathbf{128}.
\end{equation}
Descending through the branching chain $\mathrm{Spin}(16) \supset \mathrm{Spin}(14) \supset \mathrm{Spin}(4) \times \mathrm{Spin}(10)$, the 14-dimensional Observerse vector representation decomposes as $\mathbf{14} \to (\mathbf{4},\mathbf{1}) \oplus (\mathbf{1},\mathbf{10})$. Passing to the diagonal Lorentz subgroup $\mathrm{Spin}(4)_\Delta \hookrightarrow \mathrm{Spin}(4) \times \mathrm{Spin}(10)$ via the symmetric metric fiber representation $\rho = (\mathbf{3},\mathbf{3}) \oplus (\mathbf{1},\mathbf{1})$, the Observerse vector decomposes into:
\begin{equation}
...
\end{equation}
Thus, $\chi$ is rigorously proven to be the unique gauge-invariant conformal trace $(\mathbf{1},\mathbf{1})$ of the metric fiber over four-dimensional spacetime. In the static vacuum, the topological coherence functional is bounded at the fixed point $\chi_0 = 1/\sqrt{2}$.
