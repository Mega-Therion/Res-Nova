...
| **D6** Relativistic Stability | ⚠️ [P/O] | **Ghost-free re-check CLOSED 2026-09-12, verified twice independently** (`TARGET_D1_SUPPLEMENT` §5 and `TARGET_D6_D8_D9_SUPPLEMENT` rows 9–11): J′>0, J″>0, J′+2𝒴J″>0 identically for all 𝒴>0 under F_std; Hamiltonian bounded-below also PASS (row 10). Remaining D6 scope: Λ_SC ≈ 1.8 meV strong-coupling scale vs fifth-force/EP experiments — 7.25 orders above the old claim, NOT yet checked [O]; J-normalization muddle across D7 §2.1/D9 §2 [O]; AeST superluminality [O]. `SkordisZlosnikEmbedding.lean` is [X]-stale on μ_dual (row 3) | ⚠️ |
...
- The O4 redshift test (commit `3d1355b`) found H_const (a₀ = constant) favoured over H_horizon (a₀ = ξcH(z)) at 5.9σ — the horizon interpretation is **disfavoured by data**. **SUPERSEDED 2026-09-12:** that verdict was extracted under the falsified μ_dual. Re-extracted under μ_std (V2 protocol, harness validated against V1 to 6 decimals): **INCONCLUSIVE, Δχ² = +4.24 (2.06σ), and χ²_horizon (8.64) now sits below χ²_const (12.89)** — the horizon-tied model fits slightly better under the live closure. The a₀(z) question is OPEN again; no live empirical anchor distinguishes the hypotheses at intermediate z.
...
- Q: "Is a₀ = cH?"
  A: The 2π KMS cancellation is proved (ξ=1). But cH₀/a₀ ≈ 5.67, and the O4 redshift test is **inconclusive**, not decisive. The 5.9σ disfavouring was extracted under the now-falsified μ_dual; re-extracted under the live μ_std it gives χ²=12.886 vs 8.645, Δχ²=+4.242, 2.06σ — and the sign of the preference reverses. A sensitivity scan shows the verdict flips across the ±30% a₀-extraction range, so the test is hostage to the SPARC a₀ central value's own μ-dependence. The theory treats a₀ as a fundamental constant; the horizon coincidence is noted but not claimed as a derivation, and the redshift test currently neither supports nor excludes it. See `PREREG_A0_OF_Z_V2.md` / `A0_OF_Z_REPORT_V2.json` (2026-09-12).
...
- Present D6 as "kinetic-sector ghost-free re-check CLOSED (identical, 2026-09-12, TARGET_D1_SUPPLEMENT §5); full Hamiltonian analysis pending". D7, D9 remain "framework corrected, re-verification pending" — D9's factor-of-2 normalization fix needs re-verification against its own Lean file
...
8. **O4 — the 5.9σ claim is [X]-retracted (2026-09-12), so this item is no longer "get it to [P]".** Two independent defects: (a) it was extracted under the falsified μ_dual, and the μ_std re-run is INCONCLUSIVE at 2.06σ with the preference *reversed*; (b) the 20 points in `a0_of_z.py` are still hardcoded with no provenance or fetch script, flagged non-Gate-1-compliant by the repo's own `gate2_inference.py`. The provenance work remains necessary but is no longer sufficient: re-extracting a₀(z) under μ_std is the prerequisite for any re-claim, and the figure to withdraw wherever it still appears is 5.9σ, not to restore it.
9. ~~O6 CI automation~~ — **closed 2026-09-08.** `lean-gate` runs `verify_all_proofs.sh` on push/schedule/dispatch; 39/39 targets, gate list verified identical to lakefile roots. Residual is not the gate but the Mathlib cache endpoint's stall rate, which the daily cron now samples automatically (3/3 green so far) instead of via one-off manual walks.
