...
**Document Status:** Protocol, registered concurrently with first results (ordering not evidenced -- see `PROVENANCE.md`)  
...
**Output Artifact:** generated on demand; no result artifact is committed (see `PROVENANCE.md`)  

> **CONTAINMENT NOTICE.** The 20 rows currently retained in `a0_of_z.py` have
> **undocumented provenance**: no source table, retrieval record, native
> catalogue identifiers, checksum, or selection rule exists for them in the
> repository history. They are *not* described as real observational data, and
> *not* as fabricated, synthetic, or placeholder -- no evidence establishes any
> of those labels. Nothing computed from them may be cited, quoted as a
> significance, or used to advance a claim level. O4 remains **D0**. See
> `03_observer_jwst/PROVENANCE.md`.
...
### D. Model Predictions (ordering relative to first results is not evidenced -- see `PROVENANCE.md`):
```
========================================================================================================
...
1. **Tier 1 (INTENDED primary high-$z$ sample; NOT the source of any rows currently in the harness):** Published VLT/MUSE 3D kinematics (MUSE-DARK III / MUSE HUDF / Bouché et al. 2021 / Mercier et al. 2022) covering star-forming disk galaxies at $0.33 \le z \le 1.44$.
