...
The tracked Lean~4 sources under \texttt{05\_lean\_formalization/} comprise \textbf{28 modules},
...
O6 --- closed 2026-09-08. Cold-machine condition: \texttt{VERIFICATION\_RUN\_008} (2026-09-07, commit \texttt{d482823}; fresh container, no pre-existing \texttt{.lake/packages} or Mathlib host cache, \texttt{lake exe cache get} performing a genuine network fetch). The gate \texttt{verify\_all\_proofs.sh} elaborated all 32 declared targets with exit status 0, no \texttt{sorry} in proof code, and axiom footprints confined to $\{\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}\}$, under Lean \texttt{v4.33.0-rc1} and Mathlib \texttt{5eec30bc}. The gate transcript, cache-fetch transcript, per-target results, toolchain and Mathlib revisions, and checksums are archived in \texttt{VERIFICATION\_RUN\_008/01\_lean/}. CI-release-gate condition: the \texttt{lean-gate} workflow runs the gate on every push, on schedule, and on dispatch, and was 3/3 green including the cold scheduled run of 2026-09-08, at the then-declared 39 targets. The gate has since grown to 43 targets (SU(2) envelope rungs, merged 2026-09-09, PR \#39, independently verified at merge; CI green on every push), so current runs cover 43/43. Residual infrastructure caveat, not an open reproduction question: the Mathlib prebuilt-cache endpoint is sometimes-cold-fetchable, not reliably so, and the daily cron bounds the stall rate with data.
...
\texttt{CoshCosmology.lean} & \texttt{cosh\_curvature\_independent\_of\_Z0}, \texttt{cad\_sq\_early\_saturation}, \texttt{cad\_sq\_pos}, \texttt{late\_time\_equation\_of\_state}, \texttt{quadratic\_exceeds\_threshold}, \texttt{quadratic\_bounded\_by\_one} \\
...
\texttt{PPNLimits.lean} & \texttt{fractional\_deviation\_eq}, \texttt{mu\_newtonian\_limit}, \texttt{solar\_system\_precision\_bound}, \texttt{cassini\_radar\_delay\_satisfied} \\
...
\end{tabularx}
\caption{Headline declarations in the 22 tracked Lean~4 source modules, enumerated from
