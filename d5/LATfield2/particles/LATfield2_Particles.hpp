                   ...
                   int noutput)
{
      ...
      if(parallel.grid_size()[1]%2!=0)
      {
          ...
          {
            /* D5 fix: single-rank dim -> self-exchange; blocking recv-before-send
               on self can never complete in one thread. Send (buffered) first. */
            if(parallel.grid_size()[1]==1)
            {
              //send
              parallel.send_dim1( &bufferSize[3], 3,0);
              for(int i=3;i<6;i++)
     ...
     //send z
  	...
  	if(parallel.grid_size()[0]%2!=0)
  	{
  		...
  		{
          /* D5 fix: single-rank dim -> self-exchange; send (buffered) first. */
