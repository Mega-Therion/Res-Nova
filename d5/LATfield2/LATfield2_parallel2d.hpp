...
Parallel2d::Parallel2d() : neverFinalizeMPI(false)
{
	...
	/* D5 fix: buffered sends make all blocking send/recv orderings deadlock-free */
	{
		int bs = 256*1024*1024;
		void *bbuf = malloc(bs);
		if (bbuf) MPI_Buffer_attach(bbuf, bs);
	}
#else
    world_comm_ = MPI_COMM_WORLD;
	MPI_Init( &argc, &argv );
...
{
	MPI_Bsend( array, len*sizeof(Type), MPI_BYTE, to, 0, world_comm_ );
}
...
{
    MPI_Bsend( array, len*sizeof(Type), MPI_BYTE, to, 0, dim0_comm_[grid_rank_[1]] );
}
...
template<class Type> void Parallel2d::send_dim1(Type* array, int len, int to)
{
    ...
    MPI_Bsend( array, len*sizeof(Type), MPI_BYTE, to, 0, dim1_comm_[grid_rank_[0]] );
}
