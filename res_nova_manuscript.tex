...
\begin{abstract}
We investigate the dual-channel AQUAL potential $\mathcal{F}_{\text{dual}}(x) = \tfrac{1}{2}x^2 - x + \ln(1+x)$, demonstrate that its simple constitutive ratio $\mu(x) = x/(1+x)$ is ruled out by solar-system constraints, and establish the viable covariant completion under the standard interpolation $\mu_{\text{std}}(x) = x/\sqrt{1+x^2}$ alongside a reproducible SPARC benchmark. We carry the surviving branch through to a relativistic completion with quantitative solar-system and cosmological consequences. Given four structural constraints --- constitutive-relation form, Pad\'e$[1/1]$ minimality, the MOND boundary conditions, and dual-channel splitting --- $\mathcal{F}_{\text{dual}}$ is uniquely determined; $\mu$ is the functional inverse of the Bayesian odds ratio and obeys the Fisher identity $\mathcal{F}'(x)^2\mathcal{I}(\mu(x)) = x^3$. The necessity of the Pad\'e constraint is not derived, so the uniqueness is conditional. Specifying the Skordis--Z\l{}o\'snik free function as $\mathcal{F}(\mathcal{K}) = \mathcal{F}_{\text{dual}}(\sqrt{\mathcal{K}})$ leaves the Friedmann background unmodified and places the cosmological background on the Newtonian branch, giving a linear screening ratio $\mathcal{F}''/\mathcal{F}' \approx 0.00233$: the $\sim 76\times$ structure overproduction of a uniform MOND enhancement falls to $0.23\%$ in linear growth. \textbf{Correction (2026-09-12): the solar-system claim below is retracted; see the correction subsection preceding \S\ref{sec:covariant}'s original solar-system subsections.} $\mu(x)=x/(1+x)$ produces a constant, unscreened anomalous acceleration that predicts Mercury perihelion precession $\sim10^3\times$ the observed bound (INPOP10a/EPM2011) — a genuine falsification, not resolved by Vainshtein screening, which has no perturbation to act on for a constant offset. The corrected interpolation function $\mu_{\text{std}}(x)=x/\sqrt{1+x^2}$ does not have this failure mode. The completion is ghost-free with $c_T = c$ (this claim is independent of the interpolation-function choice and stands). Across 171 SPARC galaxies we measure $a_0 = (1.116 \pm 0.161)\times 10^{-10}\,\mathrm{m\,s^{-2}}$. A pre-registered test on 20 intermediate-redshift galaxies under $\mu_{\text{std}}$ is inconclusive ($\Delta\chi^2 = +4.24$, $2.06\sigma$; the earlier $5.9\sigma$ figure under $\mu_{\text{dual}}$ is retracted as closure-dependent); whether $a_0$ evolves with redshift remains open. The formal core is machine-checked in Lean~4 (18 modules, no \texttt{sorry}). Non-linear structure formation, exact PPN parameters, and cluster scales remain open.
\end{abstract}
...
\input{publication/canonical_citation}
\canonicalcitation
  {Res Nova: Geometrically Ordered Dynamics and Information Tension}%
  {10.5281/zenodo.21539453}%
  {10.5281/zenodo.22079177}%
  {v1.9.0}%
  {This text supersedes the deposited \texttt{v1.6.2} record, DOI
   \href{https://doi.org/10.5281/zenodo.21969121}{10.5281/zenodo.21969121},
   which is a GitHub release archive and remains unmodified as a historical
   document; see the Note Added below. Earlier manuscript versions under the
   concept DOI are 21660856 (5.0.0), 21623111 (4.0.0), 21583646, 21544746 and
   21539454. Release archives are a separate lineage from the manuscript and
   should not be cited as the paper.}

\noindent\textbf{Artifacts.} Verified artifact references
...
Both limits and the underlying algebraic identities are machine-certified in Lean~4 (\texttt{DualChannelDerivation.lean}, \texttt{PPNLimits.lean}). The Newtonian limit is additionally certified as a formal filter statement, \texttt{PPNLimits.mu\_newtonian\_limit} : $\mathrm{Filter.Tendsto}\,(\mu)\,\mathrm{atTop}\,(\mathrm{nhds}\,1)$, proved from \texttt{fractional\_deviation\_eq} in the 43-target gate (added 2026-09-09, D3.1; green cold in CI). \textbf{Scope note:} the Lean module certifies the channel-balance identity $x - \tfrac{x}{1+x} = \tfrac{x^2}{1+x}$ and the bounds $\mu(x)<x$, $\mu(x)<1$; it does not carry out symbolic differentiation of $\mathcal{F}_{\text{dual}}$. The step from Eq.~(\ref{eq:fdual}) to Eq.~(\ref{eq:fprime}) is elementary calculus verified by hand and by computer algebra, not by Lean.
...
\medskip\noindent\textbf{Branch note.} The theorems of this section are proved
\emph{about} $\mathcal{F}_{\text{dual}}$ and remain true as mathematics. They are
not claims about the live physical model: $\mu(x) = x/(1+x)$ is ruled out by the
solar-system screening bound, and the surviving branch is
$\mu_{\text{std}}(x) = x/\sqrt{1+x^2}$. Read each $\mathbf{[P]}$ here as
``proved about $\mathcal{F}_{\text{dual}}$''.\medskip

\textbf{Theorem (Pad\'e uniqueness) [P, about the retired branch].} $\mu(x) = x/(1+x)$ — \textbf{ruled out} physically, see the branch note above — is the unique Pad\'e $[1/1]$ rational function $\alpha x/(\beta + \gamma x)$ with $\alpha,\beta,\gamma>0$ satisfying $\mu(0)=0$, $\mu(\infty)=1$, and $\mu'(0)=1$.
...
\end{equation}
with $\mu(x) = x/(1+x)$ $\mathbf{[P]}$ — proved about the \textbf{retired} dual branch; the live interpolation is $\mu_{\text{std}}(x)=x/\sqrt{1+x^2}$. Under spherical symmetry this reduces to the algebraic relation $g \cdot \mu(g/a_0) = g_{\text{bar}}$, giving the closed-form acceleration:
\begin{equation}
\label{eq:g_predict}
...
\textbf{Result} $\mathbf{[X]}$ \textbf{(retracted 2026-09-20), superseded by} $\mathbf{[O]}$: the original run gave $\chi^2(H_{\text{const}}) = 5.37$, $\chi^2(H_{\text{horizon}}) = 40.49$, $\Delta\chi^2 = -35.1$, reported as $5.9\sigma$ in favour of constant $a_0$ (\texttt{A0\_OF\_Z\_REPORT.json}). That run froze \emph{both} models on $\mu(x) = x/(1+x)$, which was falsified on 2026-09-12; the result is therefore closure-sensitive and is retracted. Re-run under the live $\mu_{\text{std}}$ closure (\texttt{A0\_OF\_Z\_REPORT\_V2.json}; \texttt{PREREG\_A0\_OF\_Z\_V2.md}): $\chi^2(H_{\text{const}}) = 12.886$, $\chi^2(H_{\text{horizon}}) = 8.645$, $\Delta\chi^2 = +4.242$, $2.06\sigma$ --- \textbf{inconclusive}, and note the sign has reversed. Neither reading is excluded, and the horizon interpretation of $a_0$ is \emph{not} disfavoured by the current intermediate-redshift kinematic data.
...
\qquad \frac{\mathcal{F}''(\mathcal{K}_0)}{\mathcal{F}'(\mathcal{K}_0)} = \frac{1}{2x_0^2(1+x_0)} \approx 0.00233 . \quad \mathbf{[P]}
\end{equation}
The MOND enhancement of linear growth is thus $\sim0.23\%$, not the order-unity factor that a naive uniform-MOND treatment would give.

\section{Solar System and PPN Limits (D3)}
\label{sec:solar}

\subsection{Correction (2026-09-12): $\mu=x/(1+x)$ is falsified for solar-system use}

\textbf{The three subsections below (as originally submitted) are retracted and replaced.}
An independent re-derivation found that $\mu_{\text{dual}}(x)=x/(1+x)$ produces a
\emph{constant}, radius-independent anomalous acceleration $\tilde a_0=(1+\lambda_s)a_0\ge a_0$
in the covariant (AeST) embedding used in this manuscript. This is the exact spherically
symmetric solution of the field equation on the SZ tracking branch ($J'(\mathcal{Y})\to\lambda_s
\ne0$), not a response to a perturbation — so there is nothing for Vainshtein screening to
suppress. The originally submitted $Q_2\approx4.9\times10^{-29}\,\text{s}^{-2}$ figure numerically
coincides with the dipole ($\ell=1$) tidal scale reached by a different route than the one
derived below Eq.~\ref{eq:q2pred_orig} (which invokes $\mathcal{F}''/\mathcal{F}'\times$Vainshtein
suppression, an estimate that does not survive scrutiny of which action is actually in use in
\S\ref{sec:covariant}); its conclusion (safety) does not follow from its stated derivation.

The correct, load-bearing discriminant is not $Q_2$ at all but \textbf{anomalous planetary
perihelion precession}, $\Delta\varpi = 2\pi\tilde a_0 r^2/(GM)$ per orbit (verified by direct
numerical orbit integration). At Mercury this predicts $1.5\,\text{arcsec/century}$ (retrograde,
at $\lambda_s\to0$; larger for $\lambda_s>0$) against a real bound of
$\sim10^{-3}\,\text{arcsec/century}$ from INPOP10a/EPM2011 supplementary-precession fits — a
violation by $\sim10^3$, growing outward (Saturn: $7.5\,\text{arcsec/century}$). This is
$r$-independent and not removable by tuning $\lambda_s$, $c_1$, $c_2$, or $c_3$: it is a
property of the $1/x$ tail of $\mu_{\text{dual}}$ alone, and (per Milgrom, arXiv:1205.1317)
this failure mode is general to interpolation functions with this tail, not specific to this
completion.

\textbf{The fix is a different interpolation function, not a different screening mechanism.}
$\mu_{\text{std}}(x)=x/\sqrt{1+x^2}$ has no constant tail — its large-$x$ residual decays as
$a_0^2/(2g_N)$, safely below the precession bound by several orders of magnitude. This changes
the SPARC galaxy-scale comparison of \S\ref{sec:sparc}: under $\mu_{\text{std}}$, this theory's
interpolation function is identical to standard MOND's, and the comparison reduces to $a_0$
provenance (derived vs.\ fitted) alone. See the companion correction document
\texttt{MU\_DUAL\_SOLAR\_SYSTEM\_RECONCILIATION\_2026-09-12.md} in the repository for the full
derivation. \textbf{Open $\mathbf{[O]}$}: whether the constant $\tilde a_0$ residual could
partially be absorbed by standard ephemeris-fit $GM$/semi-major-axis degeneracies has not been
checked against a real ephemeris pipeline; the wrong radial scaling ($\tilde a_0$ is not
$\propto 1/r^2$) makes this unlikely but it is not independently verified.

\subsection*{[RETRACTED] The inner solar system is Newtonian by construction}

At Earth's orbit the dimensionless MOND correction is $a_0/g \approx 2\times10^{-8}$, roughly $1137\times$ below the Cassini sensitivity $|\gamma-1| < 2.3\times10^{-5}$ $\mathbf{[D]}$. This holds for \emph{any} MOND interpolation function, since all of them satisfy $\mu\approx1$ deep in the Newtonian regime; it is therefore not a test that discriminates $\mu = x/(1+x)$ from alternatives. \textbf{Retracted: this fractional argument does not bound perihelion precession.} The orbital lever arm $r^2/(GM)\approx2.5\times10^{10}\,\text{s}^2/\text{m}$ converts the same $\sim2\times10^{-8}$ fractional effect into an absolute anomalous acceleration whose integrated effect over an orbit is $\sim10^3\times$ the observed precession bound. Fractional smallness of $a_0/g$ does not imply the theory is safe; see the correction above.

\subsection*{[RETRACTED] The external field effect and the $Q_2$ constraint}
...
\end{equation}
consistent with zero, giving a bound $|Q_2| < 3.4\times10^{-27}\,\text{s}^{-2}$. For classical AQUAL/QUMOND this is in $3$--$15\sigma$ tension with the external field effect implied by galactic rotation curves $\mathbf{[C]}$. \textbf{The non-relativistic dual-channel action does not resolve this tension}; the resolution must come from the covariant completion. \textbf{Retracted: $Q_2$ is not the discriminating observable for this failure mode} — the true $\ell=2$ quadrupole term from $\tilde a_0$ is $\sim10^{-9}$ orders below this bound; Mercury precession (a monopole/dipole effect) is what falsifies $\mu_{\text{dual}}$, as corrected above.

\subsection*{[RETRACTED] Vainshtein screening}
...
The RMOND embedding of \S\ref{sec:rmond} does not apply MOND universally. Because the cosmological background sits on the Newtonian branch, the linear enhancement is $\xi \approx 1.0023$ rather than $\xi = 2$: the overproduction is suppressed by a factor $\sim429$, from $76\times$ to $0.23\%$ $\mathbf{[P]}$. A sub-percent modification of linear growth is compatible with CMB and large-scale-structure constraints, which is the structural reason RMOND reproduces the acoustic peaks where non-relativistic MOND cosmologies do not.

\begin{tcolorbox}[colback=boxbg,colframe=primary,title=\textbf{Limits of this result $\mathbf{[O]}$}]
Three limits are stated explicitly. (i) The $0.23\%$ figure is \emph{linear} theory; the non-linear regime requires N-body simulation in the Thomas et al.~\cite{Thomas2023} formulation and has not been run. (ii) The linear CMB agreement is inherited from \cite{SkordisZlosnik2021}; it has \emph{not} been recomputed with $\mathcal{F}_{\text{dual}}$ in a modified Boltzmann code. (iii) Cluster scales remain untested (O5). We do not claim a complete cosmology --- we claim that the specific overproduction obstruction is removed at linear order.
\end{tcolorbox}
...
The Lean~4 suite (\texttt{05\_lean\_}\allowbreak\texttt{formalization/}) comprises \textbf{18 modules}, 0 \texttt{sorry}, axioms $\{\texttt{propext}$, $\texttt{Classical.choice}$, $\texttt{Quot.sound}\}$, Lean \texttt{v4.33.0-rc1}, Mathlib pinned at \texttt{5eec30bc}. Gate: \texttt{verify\_}\allowbreak\texttt{all\_proofs.sh}. The directory also contains Lean modules belonging to adjacent projects that share this package and its Mathlib pin; they are built and gated identically but are not results of this paper and are not enumerated in Table~\ref{tab:lean_modules}. They are declared in \texttt{05\_lean\_}\allowbreak\texttt{formalization/}\allowbreak\texttt{ADJACENT\_MODULES.txt} so the scoping is auditable.

\textbf{Important provenance note:} O6 (fresh-clone reproduction) --- closed 2026-09-08. Cold-machine condition: \texttt{VERIFICATION\_RUN\_008} (2026-09-07, commit \texttt{d482823}; fresh container, no pre-existing \texttt{.lake/packages} or Mathlib host cache, genuine network fetch; 32/32 targets PASS, exit 0; artifacts in \texttt{VERIFICATION\_RUN\_008/01\_lean/}). CI-release-gate condition: the \texttt{lean-gate} workflow runs \texttt{verify\_all\_proofs.sh} on every push, on schedule, and on dispatch, and was 3/3 green including the cold scheduled run of 2026-09-08 at the then-declared 39 targets. The honest claim at closure was ``39/39 targets pass from a cold fetch, gated in CI on every push.'' The gate has since grown to 43 targets (SU(2) envelope rungs, merged 2026-09-09, independently verified at merge; CI green on every push), so current runs cover 43/43. Residual infrastructure caveat, not an open reproduction question: the Mathlib prebuilt-cache endpoint is sometimes-cold-fetchable, not reliably so, and the daily cron bounds the stall rate with data.
...
\texttt{YettParadigm.lean} & Positivity of an \emph{assumed} spectral gap $\kappa^2 \le \lambda_1-\lambda_0$; bounded dissipative trajectories $\mathbf{[P]}$. \texttt{chiral\_phase\_stable} projects its own hypothesis: $\theta \ge 0.70$ is assumed, not proved $\mathbf{[O]}$. \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}
    \item \textbf{O1 --- $a_0$--Horizon Identity:} KMS thermal equilibrium formally yields $a = cH_0$ ($\xi = 1$) $\mathbf{[P]}$ (\texttt{HorizonScale.lean}). The $1/(2\pi)$ divisor remains unexplained $\mathbf{[O]}$. The pre-registered $a_0(z)$ test is \textbf{inconclusive} under the live closure ($\Delta\chi^2 = +4.242$, $2.06\sigma$) $\mathbf{[O]}$ (O4); the earlier $5.9\sigma$ reading is retracted $\mathbf{[X]}$. The horizon-tied interpretation is neither supported nor killed pending larger samples.
    \item \textbf{O2 --- $\Omega_\Lambda = \ln 2$:} An asymptotic entropy boundary condition, not a dynamical prediction. A full curved-spacetime covariant derivation has not been written. A pre-registered consistency check against the Pantheon+ sample (1590 SNe after the official quality cut; \texttt{O3\_PANTHEONPLUS\_RESULT.json}) finds $\Omega_\Lambda = \ln 2$ within $1.37\sigma$ of the best-fit value: consistent, but consistency of a definition with data is not evidence for the derivation that remains unwritten $\mathbf{[D]}$.
    ...
    \item \textbf{O4 --- $a_0(z)$ Redshift Test:} \textbf{Executed, inconclusive} $\mathbf{[O]}$. Pre-registered test on 20 MUSE-DARK III galaxies. The first run ($5.9\sigma$ for $H_{\text{const}}$) is retracted $\mathbf{[X]}$: both models were frozen on the falsified $\mu(x) = x/(1+x)$. Under $\mu_{\text{std}}$, $\Delta\chi^2 = +4.242$ ($2.06\sigma$), which excludes neither hypothesis. Extension to JWST/NIRSpec high-$z$ samples pending. (Note: O4 in the original numbering referred to CMB/structure growth; the redshift test was added as a pre-registered protocol and has been completed. The CMB/structure-growth frontier remains open.)
    ...
    \item \textbf{O8 --- Non-Linear Structure Formation (D5):} The $0.23\%$ linear result of \S\ref{sec:cosmo} does not extend to the non-linear regime without N-body simulation in the RMOND formulation \cite{Thomas2023}. Not run.
...
\begin{enumerate}
    ...
    \item \textbf{$a_0(z)$ Redshift Test:} \textbf{Executed, inconclusive} $\mathbf{[O]}$. Under the live $\mu_{\text{std}}$ closure the pre-registered test gives $\Delta\chi^2 = +4.242$ ($2.06\sigma$), excluding neither constant $a_0$ nor the horizon-tied $a_0(z) = \xi c H(z)$. The earlier $5.9\sigma$ exclusion is retracted $\mathbf{[X]}$ as closure-sensitive. A larger high-$z$ sample remains the way to settle it.
...
\section*{Note Added (2026-09-09): Correction of the Linear Screening Ratio}
This version corrects one published number in the deposited v1.6.2 record (Zenodo DOI 10.5281/zenodo.21969121). The linear screening ratio was previously quoted as $\mathcal{F}''/\mathcal{F}' \approx 0.004$, growth enhancement $0.4\%$, $\xi \approx 1.004$, suppression $\sim250\times$. The correct values are $\mathcal{F}''/\mathcal{F}' = 1/(2x_0^2(1+x_0)) \approx 0.00233$, growth enhancement $0.23\%$, $\xi \approx 1.0023$, suppression $\sim429\times$. The error was a missing chain-rule factor in the second derivative of $\mathcal{F}(\mathcal{K}) = \tfrac{1}{2}\mathcal{K} - \sqrt{\mathcal{K}} + \ln(1+\sqrt{\mathcal{K}})$ with respect to $\mathcal{K}$ (documented in \texttt{GHOSTFREE\_AND\_SCREENING\_CORRECTION\_2026-09-08.md}). The corrected screening is $1.7\times$ stronger than published, so the published figure was conservative against the theory: no claimed bound, verdict, or pass/fail status changes, and the ghost-freedom result is unaffected ($1/(4u(1+u)^2) > 0$ for $u > 0$, now machine-checked as \texttt{CovariantCompletion.F\_dual\_ghost\_free}). The deposited v1.6.2 record is left unmodified as a historical document; this note connects the two versions.
...
\textbf{Author contributions.} R.W.Y.\ --- theoretical derivation, Lean formalization, numerical computation, and manuscript preparation. The author takes full responsibility for all claims, derivations, and numerical results reported here. Every external citation was verified against its primary source.
\end{acknowledgments}
